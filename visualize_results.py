...
def visualize_target(target_name, df_results, df_truth_all, df_prisma):
    df_res_t = df_results[df_results['target'] == target_name].copy()
    
    # Load ground truth for this target
    # If it's hospital admissions, we need to load that specific file or filter if it's combined
    # For now, let's determine the truth file based on target name
    ...
    ax1_tw = ax1.twinx()
    ax1_tw.plot(df_prisma['date'], df_prisma['Weekly_Positive_Tests'], 'g--', label='Weekly Positive Tests', alpha=0.4)
    ...
    plt.tight_layout()
    filename = f"forecast_visualization_{target_name.replace(' ', '_')}.png"
...
def main():
    ...
    for target in targets:
        ...
        visualize_target(target, df_results, None, df_prisma)
